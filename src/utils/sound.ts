...
class SoundManager {
  ...
  public playBadgeUnlocked(): void {
    ...
    if (!ctx) return;
    try {
      // Majestic triumphant chime for digital trophy unlock
      const notes = [
        { f: 523.25, d: 0.12 }, // C5
        { f: 659.25, d: 0.12 }, // E5
        { f: 783.99, d: 0.14 }, // G5
  ...
  public playCardFlip(): void {
    ...
    try {
      const now = ctx.currentTime;
      // Soft airy mechanical flip sound
      const osc = ctx.createOscillator();
      const gain = ctx.createGain();
      osc.type = 'triangle';
  ...
  public playBubble(): void {
    ...
    try {
      ...
      osc.frequency.exponentialRampToValueAtTime(baseFreq + 350, ctx.currentTime + 0.08);
  ...
  public playVoiceChoiceDetected(): void {
    const ctx = this.getContext();
    if (!ctx) return;
    try {
      const now = ctx.currentTime;
  ...
  public startAmbientAquarium(): void {
  ...
  public stopAmbientAquarium(): void {
